import org.gradle.internal.os.OperatingSystem
import proguard.gradle.ProGuardTask

plugins {
	id("shared-game-configuration")
	alias(libs.plugins.kotlinJvm)
	alias(libs.plugins.shadow)
}

dependencies {
	implementation("com.github.knokko:update-loop:2.0.1")
	implementation("com.github.knokko:sample-profiler:1.0.0")
	for (dependency in (project.ext.get("vulkanImplementationDependencies") as ArrayList<*>)) {
		implementation(dependency)
	}
	testImplementation(libs.lwjgl.harfbuzz)
	testImplementation(libs.lwjgl.stb)
	implementation(libs.lwjgl.sdl)
	implementation(libs.lwjgl.zstd)
	implementation(project(":content"))
	implementation(project(":audio"))
	implementation(project(":input"))
	implementation(project(":renderer"))
	implementation(project(":state"))
	implementation(project(":vk2d"))
	for (natives in (project.ext.get("lwjglNatives") as ArrayList<*>)) {
		testRuntimeOnly("org.lwjgl:lwjgl-harfbuzz::$natives")
		testRuntimeOnly("org.lwjgl:lwjgl-stb::$natives")
		runtimeOnly("org.lwjgl:lwjgl-sdl::$natives")
	}
}

tasks.shadowJar {
	manifest {
		attributes["Main-Class"] = "mardek.game.MardekGameKt"
		attributes["Enable-Native-Access"] = "ALL-UNNAMED"
		attributes["Add-Exports"] = "java.base/jdk.internal.misc"
	}
}

tasks.register("proguard", ProGuardTask::class) {
	dependsOn("shadowJar")
	injars("build/libs/game-all.jar")
	outjars("build/libs/mardek.jar")

	// I need java.base, and LWJGL needs jdk.unsupported (Unsafe)
	libraryjars("${System.getProperty("java.home")}/jmods/java.base.jmod")
	libraryjars("${System.getProperty("java.home")}/jmods/jdk.unsupported.jmod")

	// I just want to remove unused LWJGL classes and Kotlin std classes;
	// I do NOT want to obfuscate the game, since that will ruin stacktraces
	dontobfuscate()
	dontoptimize()

	// LWJGL system is magical, and will segfault if we remove its stuff
	keep("class org.lwjgl.system.** {*;}")

	// Due to some bitser reflection magic, proguard could remove important stuff
	keep("class mardek.** {*;}")
	keep("class com.github.knokko.bitser.** {*;}")

	// These suggestive annotations are not interesting
	dontwarn("org.jspecify.annotations.**")

	// This game doesn't use OpenXR (virtual reality)
	dontwarn("com.github.knokko.boiler**.xr**")
	dontwarn("com.github.knokko.boiler.exceptions.OpenXrFailureException")

	// This game uses SDL rather than GLFW
	dontwarn("org.lwjgl.glfw.**")

	// We don't use VMA
	dontwarn("org.lwjgl.util.vma.**")

	// These classes are only used during importing; not in the game itself
	dontwarn("com.github.knokko.vk2d.resource.Vk2dResourceWriter**")
	dontwarn("com.github.knokko.vk2d.text.FontHelper**")

	// These classes use java.awt.BufferedImage, which won't be present in the final game jar.
	// However, these classes won't be used in-game, so it doesn't matter.
	dontwarn("com.github.knokko.text.renderer.cpu.BufferedImageTextRenderer")
	dontwarn("com.github.knokko.compressor.Kim2**pressor")
	dontwarn("mardek.assets.area.ParsedTile")
	dontwarn("mardek.assets.area.ParsedTilesheet")

	// Some methods of MappedVkbBuffer/ImageCoding use java.awt.BufferedImage, but it's fine since I don't call them
	dontwarn("com.github.knokko.boiler.buffers.MappedVkbBuffer")
	dontwarn("com.github.knokko.boiler.utilities.ImageCoding")

	// We don't need all serializers
	dontwarn("kotlinx.serialization.**")
}

tasks.register("run", JavaExec::class) {
	group = "Run"
	description = "Runs the game without validation layer (VK 1.0)"

	classpath(sourceSets.main.get().runtimeClasspath)
	mainClass = "mardek.game.MardekGameKt"

	jvmArgs("--add-exports", "java.base/jdk.internal.misc=ALL-UNNAMED")
	jvmArgs("--enable-native-access", "ALL-UNNAMED")

	if (OperatingSystem.current().isMacOsX()) {
		jvmArgs("-XstartOnFirstThread")
		systemProperty("org.lwjgl.librarypath", "/usr/local/lib")
		systemProperty("org.lwjgl.vulkan.libname", "libvulkan.1.dylib")
	}
}

tasks.register("runBasicValidation", JavaExec::class) {
	group = "Run"
	description = "Runs the game with the validation layer (VK 1.0, no GPU-assisted validation)"

	classpath(sourceSets.main.get().runtimeClasspath)
	mainClass = "mardek.game.MardekGameKt"
	args("basic-validation")

	if (OperatingSystem.current().isMacOsX()) {
		jvmArgs("-XstartOnFirstThread")
		systemProperty("org.lwjgl.librarypath", "/usr/local/lib")
		systemProperty("org.lwjgl.vulkan.libname", "libvulkan.1.dylib")
	}
}

tasks.register("runGpuValidation", JavaExec::class) {
	group = "Run"
	description = "Runs the game with the validation layer (VK 1.2, with GPU-assisted validation)"

	classpath(sourceSets.main.get().runtimeClasspath)
	mainClass = "mardek.game.MardekGameKt"
	args("gpu-validation")

	if (OperatingSystem.current().isMacOsX()) {
		jvmArgs("-XstartOnFirstThread")
		systemProperty("org.lwjgl.librarypath", "/usr/local/lib")
		systemProperty("org.lwjgl.vulkan.libname", "libvulkan.1.dylib")
	}
}

tasks.register("runApiDump", JavaExec::class) {
	group = "Run"
	description = "Runs the game with Vulkan API dump and validation"

	classpath(sourceSets.main.get().runtimeClasspath)
	mainClass = "mardek.game.MardekGameKt"
	args("api-dump", "basic-validation")

	if (OperatingSystem.current().isMacOsX()) {
		jvmArgs("-XstartOnFirstThread")
		systemProperty("org.lwjgl.librarypath", "/usr/local/lib")
		systemProperty("org.lwjgl.vulkan.libname", "libvulkan.1.dylib")
	}
}
