	...
	for (rawSelection in nestedList) {
		...
		val monsters = rawSelection.subList(0, 4).map { rawName ->
			if (rawName == "null") null
				...
				val monster = battleAssets.monsters.find { it.name.lowercase(Locale.ROOT) == monsterName.lowercase(Locale.ROOT) }
				if (monster == null && battleAssets.monsters.size <= 1) null else monster!!
			}
		}
...
fun importAreaBattleAssets(battleAssets: BattleAssets, assets: AreaAssets) {
	...
	if (battleAssets.monsters.size > 1) { // In unit tests, the size <= 1
