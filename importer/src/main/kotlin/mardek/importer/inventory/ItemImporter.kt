...
private fun parseEquipment(
	...
	for (candidate in arrayOf("atk", "def", "mdef")) {
		...
		if (rawValue != null) stats.add(StatModifier(stat, parseInt(rawValue)))
	}

	var rawArmorType = rawItem["amrType"]
	if (rawArmorType == "3") rawArmorType = "\"Ar3\""
	println("raw armor type is $rawArmorType")
	...
	return EquipmentProperties(
			...
			armor = armor,
			gem = parseGemProperties(rawItem),
...
private fun parseGemProperties(rawItem: Map<String, String>): GemProperties? {
	...
	return GemProperties(
			...
			//rawName = parseFlashString(rawGem[0] as String, "raw gem name")!!
			drainHp = 0f
	)
}
...
private fun parseConsumable(combatAssets: CombatAssets, rawItem: Map<String, String>): ConsumableProperties? {
	...
	println("particle effect is $particleEffect and rawItem is $rawItem")
