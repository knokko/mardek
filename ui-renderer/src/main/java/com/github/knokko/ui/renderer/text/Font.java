...
public class Font {
	...
	private Map<String, ShapedString> shapeMap;

	public Font(byte[] ttfData) {
		this.ttfData = memAlloc(ttfData.length);
		this.ttfData.put(0, ttfData);

		try (var stack = stackPush()) {
			var pLibrary = stack.callocPointer(1);
			assertZero(FT_Init_FreeType(pLibrary));
			this.ftLibrary = pLibrary.get(0);

			var pFace = stack.callocPointer(1);
			assertZero(FT_New_Memory_Face(ftLibrary, this.ttfData, 0, pFace));
			this.ftFace = FT_Face.create(pFace.get(0));
...
//						to.y() + ") with control (" + control.x() + ", " + control.y() + ")");
	...
	public void destroy() {
		assertZero(FT_Done_Face(ftFace));
		assertZero(FT_Done_FreeType(ftLibrary));
		memFree(ttfData);
	}

	private void assertZero(int result) {
		if (result != 0) throw new RuntimeException("Result is " + result);
	}
}
