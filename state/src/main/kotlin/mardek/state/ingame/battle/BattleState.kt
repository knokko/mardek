...
class BattleState(
	...
	val enemyStates = Array(4) { index ->
		...
		CombatantState(enemy, content)
	}

	@BitField(id = 3)
	val playerStates = Array(4) { index ->
		val player = players[index] ?: return@Array null
		CombatantState(player, campaignState.characterStates[player]!!, content)
	}

	val startTime = System.nanoTime()

	@Suppress("unused")
	private constructor() : this(Battle(), emptyArray(), Content(), CampaignState())
