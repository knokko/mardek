...
layout(location = 4) in flat uint rawBlinkAddColor;
...
void main() {
	vec4 textureColor = sampleKim3(header, textureIndex, firstColor, textureCoordinates);
	float alpha = textureColor.a * opacity;
	vec4 blinkAddColor = decodeColor(rawBlinkAddColor);
	vec3 rgbColor = blinkAddColor.a * blinkAddColor.rgb + textureColor.rgb;
