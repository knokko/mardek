...
class InGameMenuRenderer(
	...
	fun beforeRendering() {
		...
		val tabRegion = transform.transform(0f, 0.08f, 0.68f, 0.84f)
		...
		if (tab is InventoryTab) this.tabRenderer = InventoryTabRenderer(
			recorder, targetImage, frameIndex, tab, tabRegion, state.campaign, resources
		)
	...
	fun render() {
		...
		if (state.menu.currentTab.inside) {
			val tabNamesRegion = transform.transform(0.7f, 0.08f, 1f, 0.92f)
