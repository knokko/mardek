...
class SkillOrItemDescriptionRenderer(
	...
	private val selectedElement = run {
		...
		if (charsPerLine < 10) null
			...
			val playerState = campaign.characterStates[battle.players[battle.onTurn!!.index]!!]!!
			SelectedElement(
	...
	fun beforeRendering() {
		...
		if (selectedElement.icon.version == 2) batch2 = resources.kim2Renderer.startBatch()
		...
		if (selectedElement.currentMastery != null && selectedElement.maxMastery != null) {
			if (selectedElement.currentMastery >= selectedElement.maxMastery) {
				batch1.requests.add(KimRequest(
					...
					scale = 0.25f * region.height / content.ui.mastered.height,
	...
	fun render() {
		...
		uiRenderer.drawString(
			resources.font, selectedElement.name, textColor, IntArray(0),
		...
		if (selectedElement.currentMastery != null && selectedElement.maxMastery != null) {
			if (selectedElement.currentMastery < selectedElement.maxMastery) {
				val masteryRenderer = ResourceBarRenderer(resources.font, uiRenderer, ResourceType.SkillMastery, AbsoluteRectangle(
		...
		renderDescription(selectedElement.description, charsPerLine) { line ->
			uiRenderer.drawString(
				resources.font, line, textColor, IntArray(0),
		...
		if (this::batch1.isInitialized) resources.kim1Renderer.submit(batch1, recorder, targetImage)
